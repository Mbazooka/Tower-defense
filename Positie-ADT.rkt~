;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;;                               Positie ADT                                  ;;
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
(define (maak-positie-adt x y)
    
  (define (x! nieuw-x)
    (set! x nieuw-x))

  (define (y! nieuw-y)
    (set! y nieuw-y))

  (define (gelijk? positie2) 
    (and (= x (positie2 'x)) (= y (positie2 'y))))

  ;; Volgende code is om een naar boven afgeronde versie van de huidige positie te maken 
  (define (ceil)
    (maak-positie-adt (ceiling x) (ceiling y)))

  ;; Volgende code is om een naar beneden afgeronde versie van de huidige positie te maken 
  (define (flo)
    (maak-positie-adt (floor x) (floor y)))

  (define (positie-copieer)
    (maak-positie-adt x y))

  (define (afstand positie2)
    (sqrt (+ (expt (- (positie2 'x) x) 2) (expt (- (positie2 'y) y) 2))))

  (define (dispatch msg)
    (cond
      ((eq? msg 'x) x)
      ((eq? msg 'y) y)
      ((eq? msg 'x!) x!)
      ((eq? msg 'y!) y!)
      ((eq? msg 'gelijk?) gelijk?)
      ((eq? msg 'ceil) ceil)
      ((eq? msg 'flo) flo)
      ((eq? msg 'positie-copieer) positie-copieer)
      ((eq? msg 'afstand) afstand) ;; Nieuw
      (else "maak-positie-adt: ongeldig bericht")))
  dispatch)
